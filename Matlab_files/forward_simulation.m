function [cost]=forward_simulation(q,rock,varargin)
...
%% reshape Q
NT=10;
Q=reshape(q,NT,NumInj+NumProd)';
load('Grid')
%% Define well type and rates
load('Controls')
Controls.InjCtrlVals  = Q(1:NumInj,:)*(Controls.Max_Rate-Controls.Min_Rate)+Controls.Min_Rate;     % CoNTrols.totTime
Controls.ProdCtrlVals = Q(NumInj+1:NumInj+NumProd,:)*(Controls.Max_ProdBhp-Controls.Min_ProdBhp)+Controls.Min_ProdBhp;  % Set rate
Controls.InjCtrlTypes  = repmat({'rate'},NumInj,1); % create a column with character 'rate'
Controls.ProdCtrlTypes = repmat({'bhp'} ,NumProd,1); % create a column with character 'rate'
...
%% Initial calculation
load so
state = initResSol(Grid, 5000*psia, horzcat(1-so,so)); % set initial saturation of two phase
...
savedetail = false; %% defalut is setting to not save all details data
 for k = 1:length(varargin)
     if strcmpi(varargin{k},'SaveDetails')
