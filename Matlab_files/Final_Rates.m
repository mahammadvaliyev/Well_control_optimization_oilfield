function [Wat_Prod Wat_Inj Oil_Prod] = Final_Rates(G, S, W, rock, fluid, simRes, ...
...
opt     = struct('OilPrice'              ,50   , ...
                 'WaterProductionCost'   ,  10 , ... % 10 
                 'WaterInjectionCost'    ,  10 , ... % 15
                 'RelativeDiscountFactor', 0.1  , ...
                 'ComputePartials',         []);
...
for step = 2 : numSteps,
    ...
    % Objective value:
%     Wat_Inj(step)  = Wat_Inj(step-1)+convertTo(sum(  wellRates(injInx)),stb);
%     Wat_Prod(step) = Wat_Prod(step-1)+convertTo( sum( -wellRates(prodInx).*f_w(prodInx) ),stb);
%     Oil_Prod(step) = Oil_Prod(step-1)+convertTo( sum( -wellRates(prodInx).*f_o(prodInx) ),stb);
   

   Wat_Inj(step)  = Wat_Inj(step-1)+dt*dFac*(sum(  wellRates(injInx)))/stb;
    Wat_Prod(step) = Wat_Prod(step-1)+ dt*dFac*(sum( -wellRates(prodInx).*f_w(prodInx)) )/stb;
    Oil_Prod(step) = Oil_Prod(step-1)+ dt*dFac*(sum( -wellRates(prodInx).*f_o(prodInx) ))/stb;
%     val(step)   = val(step-1) + dt*dFac*( - sum(  wellRates(injInx)                )*ri ...
